...
\li     \If $\proc{Find-Node}(head,middle)=\proc{Find-Node}(head,middle+\proc{Loop-Length}(head))$ \Then
\li         $right\get middle$
\li     \Else
\li         $left\get middle$
        \End
...
In the procedure \proc{Chain-Length-D}, we first use \proc{IsLoop} to find a node, say, $L$, on the loop. Then, we record two arrays, $chainArray$ and $loopArray$. The former 
