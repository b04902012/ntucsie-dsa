...
The procedure \proc{Chain-Length(c)} exploits the fact that if $chainLength>0$, then $chainLength$ is exactly the index of the last node that is not on the loop. Thus, we can use binary search to find such index.

\begin{codebox}
\Procname{$\proc{chainLengthC}(head)$}
\li $left\get 1$
\li $right\get 1$
...
\Procname{$\proc{chainLengthD}(head)$}
\li $loopNode\get isLoop(head)$
